...
class OptimizedDocument(models.Model):
    original_document = models.ForeignKey(
        OriginalDocument, on_delete=models.CASCADE, related_name="optimized_versions"
    )
...
class DocumentCustomization(models.Model):
    optimized_document = models.ForeignKey(
        OptimizedDocument, on_delete=models.CASCADE, related_name="customizations"
    )
...
class OptimizationAnalytics(models.Model):
    optimized_document = models.OneToOneField(
        OptimizedDocument, on_delete=models.CASCADE, related_name="analytics"
    )
    job_match_score = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], null=True, blank=True
    )
    interview_potential_score = models.FloatField(
...
class ImprovementSummary(models.Model):
    optimized_document = models.OneToOneField(
        OptimizedDocument, on_delete=models.CASCADE, related_name="improvement_summary"
    )
...
class KeyChange(models.Model):
    improvement_summary = models.ForeignKey(
        ImprovementSummary, on_delete=models.CASCADE, related_name="key_changes"
    )
...
class CustomizationAnalytics(models.Model):
    customization = models.OneToOneField(
        DocumentCustomization, on_delete=models.CASCADE, related_name="analytics"
    )
    effectiveness_impact = models.FloatField(
...
class CustomizationNote(models.Model):
    customization = models.ForeignKey(
        DocumentCustomization, on_delete=models.CASCADE, related_name="notes"
    )
    content = models.TextField()
