...
from resume.utils.util_funcs import get_full_url, load_document, upload_directly_to_s3
...
class DocumentsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        optimized_documents = OptimizedDocument.objects.filter(
            original_document__user=request.user, document_type=document_type, is_latest=True
        )
        ...
        for optimized in optimized_documents:
            ...
            if category not in grouped_documents:
                ...
                grouped_documents[category]["optimized"] = OptimizedDocumentSerializer(
                    optimized
                ).data
...
class DocumentUploadView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            doc_content = async_to_sync(load_document)(file_key=file_key)
            ...
            existing_document = OriginalDocument.objects.filter(
                user=request.user, category=category, document_type=document_type
            ).first()
            ...
            if existing_document:
                ...
                existing_document.content = doc_content
...
class DeleteOriginalDocumentView(APIView):
    ...
    def delete(self, request, id):
        try:
            ...
            return Response(
                {"message": "Original document and its optimized versions deleted successfully"},
                status=status.HTTP_200_OK,
            )
...
class DeleteOptimizedDocumentView(APIView):
    ...
    def delete(self, request, id):
        try:
            ...
            document.delete()
            return Response(
                {"message": "Optimized document deleted successfully"}, status=status.HTTP_200_OK
            )
